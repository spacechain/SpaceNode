...
#endif
	template <unsigned ___N> bool operator==(FixedHash<___N> const& _h) const { return isData() && toHash<___N>() == _h; }
	template <unsigned ___N> bool operator!=(FixedHash<___N> const& _s) const { return isData() && toHash<___N>() != _s; }
...
#warning ggggggggg   ggggggggg   ggggggggg
#endif
	template <class ___N> ___N toHash(int _flags = Strict) const
	{
		requireGood();
		auto p = payload();
		auto l = p.size();
		if (!isData() || (l > ___N::size && (_flags & FailIfTooBig)) || (l < ___N::size && (_flags & FailIfTooSmall)))
		{
			if (_flags & ThrowOnFail)
				BOOST_THROW_EXCEPTION(BadCast());
			else
		...
		___N ret;
		size_t s = std::min<size_t>(___N::size, l);
		memcpy(ret.data() + ___N::size - s, p.data(), s);
		return ret;
	}
